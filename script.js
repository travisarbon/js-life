        ...
        var LifeBoard = React.createClass({
                ...
                if(this.state.running && nextState.running){
                    ...
                    for(var k in nextState){
                        if(nextState.hasOwnProperty(k) &&
                           k !== 'popHistory' && k !== 'srAnnouncement' && k !== 'liveCells' &&
            ...
            componentDidMount : function(){
                ...
                this._onPopOutDismiss = function(e){
                    if(!self.state.activePopOut){ return; }
                    if(e.type === 'keydown' && e.key === 'Escape'){ self._closePopOut(); return; }
                    if(e.type === 'mousedown'){
                        var popOut = e.target.closest && e.target.closest('.pop-out-trigger');
                        if(!popOut){ self._closePopOut(); }
                    }
                };
            ...
            componentWillUnmount : function(){
                if(this._tabBarObservers){
                    this._tabBarObservers.forEach(function(obs){ obs.disconnect(); });
                }
                    ...
                    <div className="toolbar-strip">
                        ...
                        <div className="toolbar-groups">
                            ...
                            <div className="toolbar-group">
                                ...
                                {this.state.boundary !== 'unbounded' && <button type="button" className={"btn btn-toggle" + (this.state.drawMode === 'region' ? " active" : "")} onClick={this.toggleRegionMode} title="Draw/erase region bounds (B)">Region</button>}
                ...
                return (
                    <div className="tools-content">
                        <div className="btn-section">
                            ...
                            {this.state.boundary !== 'unbounded' && <div className="tool-subtype-row">
                                <label className="tool-label">Region:</label>
                                <select value={this.state.regionTool}
                                        onChange={function(e){ self.setState({regionTool: e.target.value, drawMode: 'region'}); }}>
                                    <option value="cell">Cell paint</option>
                                    <option value="line">Line</option>
                                    <option value="fill">Flood fill</option>
                                    <option value="shape-rect">Rectangle</option>
            ...
            renderCartographer : function(cs){
                ...
                var tabContent = (
                    <div className="rail-tab-content">
                        {this._buildTabContent(this.state.railTab, {sectionTitle: true})}
                    </div>
                );
            ...
            renderCartographerMobile : function(cs){
                ...
                return (
                    <div className="layout-cartographer layout-mobile">
                        {this.renderCanvas(cs)}
                        {!this.state.bottomSheetOpen && !this._statsChipHidden && this._renderStatsChip()}
            ...
            _renderFloatPanel : function(panelId, label, content){
                ...
                if(this._findGroupForPanel(panelId)){ return null; }
                ...
                return (
                    <div className={className} style={style} data-panel-id={panelId}
                        onMouseDown={function(){ self._bringPanelToFront(panelId); }}
                        onTouchStart={function(){ self._bringPanelToFront(panelId); }}
                        ...
                        <div className="float-panel-header"
                            ...
                            <button type="button" className="btn float-panel-compact-toggle"
                                onClick={function(){ self._togglePanelCompact(panelId); }}
                        ...
                        {!ps.collapsed && <div className="float-panel-body">
                            {isCompact ? this._renderCompactBody(panelId) : content}
                        </div>}
                            ...
                            return (
                                <div key={def.id} className="pop-out-trigger">
                                    <button type="button"
                                        ...
                                        onClick={def.popOut ? function(){ isOpen ? self._closePopOut() : self._openPopOut(panelId, def.id); } : def.onClick}
                                        title={def.title}>
            ...
            _getCompactDefs : function(panelId){
                ...
                switch(panelId){
                    case 'transport':
                        return [
                            {id:'play', icon: this.state.running ? 'fa-pause' : 'fa-play', title: 'Play/Pause (Space)', onClick: this.toggleGame, active: this.state.running},
                            {id:'step', icon: 'fa-step-forward', title: 'Step (.)', onClick: this.stepGame},
                            {id:'back', icon: 'fa-step-backward', title: 'Step backward (,)', onClick: this.stepBack},
                            {id:'go', icon: 'fa-fast-forward', title: 'Advance multiple generations', onClick: function(){ self.stepN(self.state.stepCount); }},
                            {id:'reset', icon: 'fa-refresh', title: 'Randomize (R)', onClick: this.resetGame},
                            {id:'empty', icon: 'fa-eraser', title: 'Clear all cells (E)', onClick: this.emptyBoard},
                            {id:'undo', icon: 'fa-undo', title: 'Undo (Ctrl+Z)', onClick: this.undo},
                            {id:'speed', icon: 'fa-tachometer', title: 'Speed', popOut: function(){ return self.renderSpeedSlider(); }}
                        ];
                    case 'board':
                        return [
                            {id:'boundary', icon: 'fa-repeat', title: 'Cycle boundary', onClick: this.toggleBoundary, active: this.state.boundary !== 'toroidal'},
                            {id:'grid-size', icon: 'fa-th-large', title: 'Grid size', popOut: function(){ return self.renderBoardSliders(); }}
                        ];
                    case 'view':
                        return [
                            {id:'fit-grid', icon: 'fa-arrows-alt', title: 'Fit Grid', onClick: this.fitView},
                            {id:'fit-cells', icon: 'fa-compress', title: 'Fit Cells', onClick: this.fitLiveCells},
                            {id:'grid', icon: 'fa-th', title: 'Grid lines (G)', onClick: this.toggleGridLines, active: this.state.gridLines},
                            {id:'trails', icon: 'fa-eye', title: 'Trails', onClick: this.toggleTrails, active: this.state.showTrails},
                            {id:'minimap', icon: 'fa-map-o', title: 'Minimap (M)', onClick: this.toggleMinimap, active: this.state.showMinimap},
                            {id:'zoom', icon: 'fa-search-plus', title: 'Zoom', popOut: function(){ return self.renderZoomSlider(); }},
                            {id:'display', icon: 'fa-paint-brush', title: 'Display settings', popOut: function(){ return self.renderDisplaySettings(); }}
                        ];
                    case 'mode':
                        var defs = [
                            {id:'draw', icon: 'fa-pencil', title: 'Draw mode (D)', onClick: this.toggleDrawMode, active: this.state.drawMode === 'paint'},
                            {id:'preset', icon: 'fa-puzzle-piece', title: 'Preset patterns (P)', onClick: this.togglePresetMode, active: this.state.drawMode === 'preset'},
                            {id:'select', icon: 'fa-mouse-pointer', title: 'Select mode (S)', onClick: this.toggleSelectMode, active: this.state.drawMode === 'select'},
                            {id:'live-paint', icon: 'fa-paint-brush', title: 'Live Paint', onClick: this.toggleLivePaint, active: this.state.livePaintMode},
                            {id:'analyze', icon: 'fa-crosshairs', title: 'Analyze', onClick: this.analyzePattern},
                            {id:'tools', icon: 'fa-wrench', title: 'Tool options', popOut: function(){ return self.renderToolsContent(); }}
                        ];
                        if(this.state.boundary !== 'unbounded'){
                            defs.splice(3, 0, {id:'region', icon: 'fa-th', title: 'Region bounds (B)', onClick: this.toggleRegionMode, active: this.state.drawMode === 'region'});
                        }
                        return defs;
                    case 'rules':
                        return [
                            {id:'rules', icon: 'fa-cogs', title: 'Rules', popOut: function(){ return self.renderRulesSection(); }}
                        ];
                    case 'stats':
                        return [
                            {id:'stats', icon: 'fa-bar-chart', title: 'Statistics', popOut: function(){ return self.renderStats(); }}
                        ];
                    case 'importExport':
                        return [
            ...
            _getPanelContent : function(panelId){
                switch(panelId){
                    case 'transport': return <div>{this.renderTransportControls(false)}{this.renderSpeedSlider()}</div>;
                    case 'board': return <div>{this.renderBoardSliders()}{this.renderBoundaryControls()}</div>;
                    case 'view': return <div>{this.renderViewControls()}{this.renderZoomSlider()}{this.renderDisplaySettings()}</div>;
                    case 'mode': return <div>{this.renderModeControls()}{this.renderToolsContent()}</div>;
                    case 'rules': return this.renderRulesSection();
                    case 'stats': return this.renderStats();
                    case 'importExport': return this.renderExportContent();
                    default: return null;
                }
            },
            ...
            _observeTabBars : function(){
                ...
                for(var i = 0; i < tabBars.length; i++){
                    (function(bar){
                        var obs = new ResizeObserver(function(){ self._checkTabBarOverflow(bar); });
                        obs.observe(bar);
                ...
                return (
                    <div className={className} style={style} data-group-id={group.id}
                        ...
                        <div className="float-panel-resize"
                            ...
                            onTouchStart={function(e){ self._startGroupResize(group.id, e); }}></div>
                    </div>
                );
            },
            ...
            _startGroupDrag : function(groupId, e){
                ...
                var end = function(){
                    ...
                    self.setState({panelGroups: groups}, function(){ self._persistLayout(); });
