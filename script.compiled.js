  ...
  (function () {
    var LifeBoard = React.createClass({
        ...
        for (var r = startR; r < endR; r++) {
          for (var c = startC; c < endC; c++) {
            ...
            if (age !== undefined) {
              ...
              ctx.fillStyle = 'rgb(' + Math.round(yR + (aR - yR) * t) + ',' + Math.round(yG + (aG - yG) * t) + ',' + Math.round(yB + (aB - yB) * t) + ')';
      ...
      paintCellDirect: function (c, r) {
        ...
        ctx.fillStyle = this._dragStatus === 1 ? 'rgb(' + theme.aliveR + ',' + theme.aliveG + ',' + theme.aliveB + ')' : theme.bg;
      ...
      onMouseDown: function (event) {
        ...
        var c = pos.c,
          r = pos.r;
        if (c < 0 || c >= this.state.cols || r < 0 || r >= this.state.rows) {
          return;
        }
      ...
      onMouseMove: function (event) {
        ...
        var hoverChanged = !!newHover !== !!ph || newHover && ph && (newHover.c !== ph.c || newHover.r !== ph.r);
        if (hoverChanged) {
          this.setState({
            hoverCell: newHover
          });
        ...
        if (this.state.drawMode === 'preset' && this.state.selectedPattern) {
          ...
          if (prev && newPos && prev.c === newPos.c && prev.r === newPos.r) {
            return;
          }
        ...
        var paintKey = r + ',' + c;
        if (this._paintedCells[paintKey] !== undefined) {
          return;
        }
      ...
      onWheel: function (event) {
        ...
        var newCS = event.deltaY < 0 ? Math.min(32, cellSize + step) : Math.max(2, cellSize - step);
        if (newCS === cellSize) {
          return;
        }
      ...
      onTouchStart: function (event) {
        ...
        if (event.touches.length === 2) {
          ...
          var t0 = event.touches[0],
            t1 = event.touches[1];
          this._pinchStart = {
            dist: Math.sqrt(Math.pow(t1.clientX - t0.clientX, 2) + Math.pow(t1.clientY - t0.clientY, 2)),
      ...
      onTouchMove: function (event) {
        ...
        if (event.touches.length === 2 && this._pinchStart) {
          var t0 = event.touches[0],
            t1 = event.touches[1];
          var newDist = Math.sqrt(Math.pow(t1.clientX - t0.clientX, 2) + Math.pow(t1.clientY - t0.clientY, 2));
          ...
          var newCS = Math.max(2, Math.min(32, Math.round(this._pinchStart.cellSize * scale)));
      ...
      toggleBoundary: function () {
        this.setState({
          boundary: this.state.boundary === 'toroidal' ? 'finite' : 'toroidal'
        });
      },
      toggleGame: function () {
      ...
      setWidth: function (e) {
        var self = this;
        this.setState({
          pendingCols: parseInt(e.target.value)
        }, function () {
          self.drawBoard();
        });
      },
      ...
      setHeight: function (e) {
        var self = this;
        this.setState({
          pendingRows: parseInt(e.target.value)
        }, function () {
          self.drawBoard();
        });
      },
      ...
      parseRLE: function (text) {
        return SimEngine.parseRLE(text);
      },
      parsePlaintext: function (text) {
        return SimEngine.parsePlaintext(text);
      },
      // ── Patterns ──────────────────────────────────────────────────────

      rotatePattern: function (cells, steps) {
        return SimEngine.rotatePattern(cells, steps);
      },
      rotateCW: function () {
        var self = this;
        this.setState({
      ...
      resetGame: function () {
        this.pushUndo();
        var newLiveCells = this.buildLiveCells(this.state.cols, this.state.rows, this.state.sparseness);
      ...
      renderStats: function () {
        ...
        if (hist.length > 1) {
          ...
          sparkline = /*#__PURE__*/React.createElement("div", {
          ...
          }, "peak " + maxPop.toLocaleString() + (this.state.sessionPeakPop > maxPop ? " · all " + this.state.sessionPeakPop.toLocaleString() : ""))), /*#__PURE__*/React.createElement("svg", {
            className: "sparkline",
            width: "100%",
            height: vbH,
            viewBox: "0 0 " + vbW + " " + vbH,
      ...
      renderMobileStatsBar: function () {
        ...
        return /*#__PURE__*/React.createElement("div", {
          ...
          className: "msb-left"
        }, 'Gen\u00a0' + this.state.generations.toLocaleString() + '\u2002Pop\u00a0' + population.toLocaleString() + trendArrow), /*#__PURE__*/React.createElement("span", {
