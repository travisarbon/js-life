  ...
  parseRLE: function (text) {
    ...
    for (var k = 0; k < data.length; k++) {
      ...
      if (ch >= '0' && ch <= '9') {
        ...
        if (ch === 'o') {
          for (var j = 0; j < n && cells.length < 200000; j++) {
    ...
    var LifeBoard = React.createClass({
        ...
        if (this._trailEnabled) {
          ...
          if (trailMap.size > 50000) {
            ...
            for (var ei = 0; ei < excess; ei++) {
              var nk = iter.next();
              if (nk.done) break;
              delKeys.push(nk.value);
            }
            for (var di = 0; di < delKeys.length; di++) {
              trailMap.delete(delKeys[di]);
            }
          }
        }
      ...
      onMouseDown: function (event) {
        ...
        if (event.button === 0 && this._minimapRect && this.state.showMinimap && this.state.drawMode !== 'select') {
          ...
          if (mm.w > 0 && mm.h > 0 && mouse.x >= mm.x && mouse.x <= mm.x + mm.w && mouse.y >= mm.y && mouse.y <= mm.y + mm.h) {
        ...
        var c = pos.c,
          r = pos.r;
        if (this.state.boundary !== 'unbounded' && (c < 0 || c >= this.state.cols || r < 0 || r >= this.state.rows)) {
          return;
        }
      ...
      onWheel: function (event) {
        ...
        var newCS = event.deltaY < 0 ? Math.min(32, cellSize + step) : Math.max(1, cellSize - step);
        if (newCS === cellSize) {
          return;
        }
      ...
      floodFillCells: function (startC, startR, liveCells, cols, rows, startAlive) {
        ...
        while (queue.length) {
          ...
          var rr = cur[0],
            cc = cur[1];
          if (!isUnbounded && (cc < 0 || cc >= cols || rr < 0 || rr >= rows)) {
            continue;
          }
      ...
      toggleMobileTools: function () {
        var self = this;
        this.setState({
          showMobileTools: !this.state.bottomSheetOpen
        }, function () {
          self.drawBoard();
        });
      },
      ...
      onTouchMove: function (event) {
        ...
        if (event.touches.length === 2 && this._pinchStart) {
          var t0 = event.touches[0],
            t1 = event.touches[1];
          var newDist = Math.sqrt((t1.clientX - t0.clientX) * (t1.clientX - t0.clientX) + (t1.clientY - t0.clientY) * (t1.clientY - t0.clientY));
        ...
        if (this.state.panMode && this._panDragging && this._panStart) {
          ...
          var displayCS = panRect.width > 0 && this._canvas.width > 0 ? cs2 * (panRect.width / this._canvas.width) : cs2;
      ...
      drawMinimapMobile: function (liveCells, cols, rows, viewX, viewY, cellSize, theme) {
        ...
        var vpOutsideM = vpCenterCm < mmMobOriginC || vpCenterCm > mmMobOriginC + cols || vpCenterRm < mmMobOriginR || vpCenterRm > mmMobOriginR + rows;
        if (vpOutsideM) {
          var mmCCm = mmMobOriginC + cols / 2,
            mmCRm = mmMobOriginR + rows / 2;
