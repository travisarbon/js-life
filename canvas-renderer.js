...
const CanvasRenderer = { // eslint-disable-line no-unused-vars
    ...
    _ensurePalette: function(theme, themeName){
        const aR = theme.aliveR, aG = theme.aliveG, aB = theme.aliveB;
        const yR = theme.youngR, yG = theme.youngG, yB = theme.youngB;
        const STEPS = 63;
    ...
    drawCells: function(ctx, liveCells, startR, startC, endR, endC, viewX, viewY, cellSize, colorPalette){
        ...
        if(liveCells.size < viewArea * 0.3){
            ...
            for(let r = startR; r < endR; r++){
                const rowData = rowLookup[r];
                if(!rowData) continue;
                for(let c = startC; c < endC; c++){
                    const age = rowData[c];
                    if(age !== undefined){
                        const ci2 = Math.min(Math.round(Math.min(age / 10, 1) * COLOR_STEPS), COLOR_STEPS);
    ...
    drawTrails: function(ctx, trailMap, startR, startC, endR, endC, viewX, viewY, cellSize, trailPalette){
        trailMap.forEach(function(val, key){
            const _rc = parseKey(key), tr = _rc[0], tc = _rc[1];
    ...
    drawGrid: function(ctx, startR, startC, endR, endC, viewX, viewY, cellSize, canvasW, canvasH, gridColor){
        ...
        ctx.lineWidth = 0.5;
        ctx.beginPath();
        for(let cv = startC; cv <= endC; cv++){
            const gx = (cv - viewX) * cellSize;
            ctx.moveTo(gx, 0); ctx.lineTo(gx, canvasH);
        }
        for(let rv = startR; rv <= endR; rv++){
            const gy = (rv - viewY) * cellSize;
    ...
    drawRegionOverlay: function(ctx, regionMask, startR, startC, endR, endC, viewX, viewY, cellSize, canvasW, canvasH, theme, boundary){
        ...
        const bm = this._regionBitmapCache;
        ...
        ctx.beginPath();
        for(let br = startR; br < endR; br++){
            for(let bc = startC; bc < endC; bc++){
                if(!RegionUtil.bitmapHas(bm, br, bc)) continue;
                const bx = (bc - viewX) * cellSize;
                const by = (br - viewY) * cellSize;
    ...
    clearRegionCells: function(ctx, regionMask, startR, startC, endR, endC, viewX, viewY, cellSize, bgColor){
        ...
        for(let r = startR; r < endR; r++){
            let runStart = -1;
            for(let c = startC; c <= endC; c++){
                const inRegion = (c < endC) && RegionUtil.bitmapHas(bm, r, c);
                if(inRegion){
    ...
    drawRegionPreview: function(ctx, previewKeys, erasing, viewX, viewY, cellSize){
        ...
        for(let i = 0; i < previewKeys.length; i++){
            ...
            const r = parseInt(key.substring(0, idx), 10) || 0;
            const c = parseInt(key.substring(idx + 1), 10) || 0;
            ctx.fillRect((c - viewX) * cellSize, (r - viewY) * cellSize, cellSize, cellSize);
        }
    },
    ...
    drawSelection: function(ctx, sel, viewX, viewY, cellSize, theme){
        if(!sel) return;
        const aR = theme.aliveR, aG = theme.aliveG, aB = theme.aliveB;
        const selType = sel.type || 'rect';
        if(selType === 'rect'){
            const sx1 = (Math.min(sel.c1, sel.c2) - viewX) * cellSize;
            const sy1 = (Math.min(sel.r1, sel.r2) - viewY) * cellSize;
            const sx2 = (Math.max(sel.c1, sel.c2) - viewX + 1) * cellSize;
            const sy2 = (Math.max(sel.r1, sel.r2) - viewY + 1) * cellSize;
            ...
            const sx1e = (Math.min(sel.c1, sel.c2) - viewX) * cellSize;
            const sy1e = (Math.min(sel.r1, sel.r2) - viewY) * cellSize;
            const sw = (Math.abs(sel.c2 - sel.c1) + 1) * cellSize;
            const sh = (Math.abs(sel.r2 - sel.r1) + 1) * cellSize;
            ctx.save();
            ctx.beginPath();
            ...
            if(sel.path && sel.path.length > 1){
                ...
                for(let fi = 1; fi < sel.path.length; fi++)
            ...
            if(sel.cells && sel.cells.length > 0){
                ...
                ctx.lineWidth = 1;
                ctx.setLineDash([3, 2]);
                const avMinC = Math.min(sel.c1, sel.c2) - viewX;
                const avMinR = Math.min(sel.r1, sel.r2) - viewY;
    ...
    drawToolPreview: function(ctx, previewCells, erasing, viewX, viewY, cellSize, theme){
        ...
        ctx.fillStyle = erasing
            ? 'rgba(200,80,80,0.45)'
            : 'rgba(' + aR + ',' + aG + ',' + aB + ',0.4)';
        for(let di = 0; di < previewCells.length; di++){
            const dpr = previewCells[di][0], dpc = previewCells[di][1];
    ...
    drawPatternPreview: function(ctx, patternName, rotation, previewPos, viewX, viewY, cellSize, theme, regionMask){
        ...
        const aR = theme.aliveR, aG = theme.aliveG, aB = theme.aliveB;
        const cacheKey = patternName + ':' + rotation;
        ...
        const pattern = this._rotatedPatternCache;
        let maxPR = 0, maxPC = 0;
        for(let pi = 0; pi < pattern.length; pi++){
            if(pattern[pi][0] > maxPR){ maxPR = pattern[pi][0]; }
            if(pattern[pi][1] > maxPC){ maxPC = pattern[pi][1]; }
        }
        const offsetPR = previewPos.r - Math.floor(maxPR / 2);
        const offsetPC = previewPos.c - Math.floor(maxPC / 2);
        const hasRegion = regionMask && regionMask.size > 0;
        ctx.fillStyle = 'rgba(' + aR + ',' + aG + ',' + aB + ',0.55)';
        for(let pj = 0; pj < pattern.length; pj++){
            const pvR = pattern[pj][0] + offsetPR;
            const pvC = pattern[pj][1] + offsetPC;
    ...
    drawRotationPreview: function(canvas, patternName, rotation, theme){
        ...
        const pattern = SimEngine.rotatePattern(PATTERNS[patternName], rotation);
        let maxR = 0, maxC = 0;
        for(let i = 0; i < pattern.length; i++){
            if(pattern[i][0] > maxR){ maxR = pattern[i][0]; }
            if(pattern[i][1] > maxC){ maxC = pattern[i][1]; }
        }
        const patRows = maxR + 1, patCols = maxC + 1;
        const pad = 4;
        const size = canvas.width;
        const cellPx = Math.max(1, Math.floor((size - pad * 2) / Math.max(patRows, patCols)));
        const offX = Math.floor((size - patCols * cellPx) / 2);
        const offY = Math.floor((size - patRows * cellPx) / 2);
